    ...
    for (int i = 0; i < 'Z' + 1; i++) {
        ...
        if (cp_GCB != Uni_PVal_GRAPHEME_CLUSTER_BREAK_Control ) {
            fprintf(stderr, printf_s, i, i, (char) i, gc, get_bool_prop(i, Bidi_Mirrored), Grapheme_Cluster_Break[cp_GCB], cp_GCB );
        }
        else {
            fprintf(stderr, printf_s_control, i, i, gc, get_bool_prop(i, Bidi_Mirrored), Grapheme_Cluster_Break[cp_GCB], cp_GCB );
        }
   }
