    for (int i = 0; i < 150; i++) {
           ...
           if (cp_GCB != Uni_PVal_GRAPHEME_CLUSTER_BREAK_Control ) {
               printf(printf_s, i, (char) i, get_bool_prop(i, Bidi_Mirrored), Grapheme_Cluster_Break[cp_GCB], cp_GCB );
           }
           else {
               printf(printf_s_control, i, get_bool_prop(i, Bidi_Mirrored), Grapheme_Cluster_Break[cp_GCB], cp_GCB );
           }
    }
